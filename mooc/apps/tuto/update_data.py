...
def uniqueSlug(newtuto):
    """construit le slug d'un tuto en garantissant sont unicité"""
    newtutoSlug = slugify(newtuto.title)[:40]
    doublon = True
    rand = ""
    while doublon:
        slugList = [re.sub(r'-\d+$', '', t.slug) for t in modelClass["tutorial"].objects.all() ]
        newtutoSlug += rand
        doublon = newtutoSlug in slugList
        rand = '-' + str(randint(1000, 9999))
    return newtutoSlug + "-" + str(newtuto.version)
...
def create_data(request):
    ...
    newinst = {
        f:v for f,v in list(res_post["create"]["tutorial"].values())[0].items() if f not in manytomanyFields["tutorial"] 
        }
    ...
    try:
        restriction_list = list(res_post["create"]["tutorial"].values())[0]['restriction']
    except KeyError:
        restriction_list = list()
    newtuto.restriction.set(restriction_list)
    newtuto.author.set([request.user,])
    newtuto.save()
    ...
    for mod, coll in res_post["update"].items():
        for i, inst in coll.items():
            ...
            if instance.first():
                ...
                inst_without_manytomanyfields = { f: v for f, v in inst.items() if f  not in manytomanyFields[mod] }
            ...
            for f, val in inst.items():
                if f in foreignKeyFields[mod]:  # cas d'un champ clé étrangère 
                    if 'new' in val:            #nouvelle clé étrangère (nouvelle page...)
                        newinst.update({ f: modelClass[f].objects.get(id= corres[f][val.replace('new-', '')]) })
                    else:                       # clé étrangère existante :
                        newinst.update({ f: modelClass[f].objects.get(id=val.replace('new-', '')) }) 

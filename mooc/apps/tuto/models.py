...
from mooc.media_file_cleaning import (
...
class Tutorial(models.Model):
    ...
    category = models.ForeignKey(
        "Category", on_delete=models.CASCADE, related_name="tutorial"
    )
    ...
    restriction = models.ManyToManyField(
        "user.Restriction", related_name="tutorial", blank=True
    )
    ...
    updated_at = models.DateTimeField(
        default=timezone.now, verbose_name="date de mise à jour"
    )
    ...
    tutobase = models.ForeignKey(
        "TutoBase", on_delete=models.CASCADE, related_name="tutorial"
    )
    ...
    def get_author_names(self):
        ...
        authorlist = [
            "{} {}".format(t.first_name, t.last_name) for t in self.author.all()
        ]
        if len(authorlist) == 1:
...
class TutoBase(models.Model):
    ...
    def get_last_published_or_archived_version(self):
        return max(
            ...
            + [
                tuto.version
                for tuto in self.tutorial.filter(Q(published=True) | Q(archived=True))
            ]
        )
...
class Page(models.Model):
    ...
    def get_all_related_objects(self):
        """liste de tous les objets qui ont une relation et que l'on souhaite dupliquer"""
        return list(
            chain(Content.objects.filter(page=self), Question.objects.filter(page=self))
        )
...
class Content(models.Model):
    ...
    def short_text(self):
        if self.contenttype == "LI":
            return "[{}]: - {}".format(
                self.get_contenttype_display(), self.get_all_listitems[0].texte
            )
        else:
...
def clone(instance):
    ...
    filefield_buffer = {
        ...
        and f.value_from_object(instance)
    }

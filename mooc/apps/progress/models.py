...
class TutoProgress(models.Model):
    ...
    user = models.ForeignKey('user.User', on_delete=models.CASCADE, related_name='tutoprogress')
    tuto = models.ForeignKey('tuto.Tutorial', on_delete=models.CASCADE, related_name='tutoprogress')
    ...
    def is_in_progress(self):
        ...
        return any( pp.finished for pp in self.set_all_pageprogress()) and not self.is_finished
    ...
    def tuto_score(self):
        ...
        return sum([pp.page_score for pp in self.set_all_pageprogress() if pp.page_score])
    
    @property 
    def tuto_max_score_done(self):
        """ Score intermédiaire max sur le questions du quiz qui ont été validées """
        return sum([pp.page_max_score for pp in self.set_all_pageprogress() if pp.finished])
    ...
    def set_all_pageprogress(self): 
        ...
        for p in self.tuto.get_all_pages:
            ...
            if created :
                pp.quiztry = 1
                pp.save()
        return PageProgress.objects.filter(user=self.user, page__tuto=self.tuto).order_by('page__page_number')
...
class PageProgress(models.Model):
    ...
    @property
    def page_score(self): # ! cas de qp pas encre créé : liste de vide ! mettre 0 par défaut !
